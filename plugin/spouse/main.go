...
func init() {
	...
	engine.OnRegex(`^添加\s?(.*)$`, zero.OnlyGroup, zero.MustProvidePicture).SetBlock(true).Limit(ctxext.LimitByGroup).
	...
	engine.OnRegex(`^ab\s?(.*)$`, zero.SuperUserPermission, zero.MustProvidePicture).SetBlock(true).Limit(ctxext.LimitByGroup).
		Handle(func(ctx *zero.Ctx) {
			keyword := ctx.State["regex_matched"].([]string)[1]
			spouseType := model.Type(keyword)
			if spouseType.String() == "" {
	...
	engine.OnRegex(`^删除\s?(.*)$`, zero.OnlyGroup).SetBlock(true).Limit(ctxext.LimitByGroup).
		Handle(func(ctx *zero.Ctx) {
			keyword := ctx.State["regex_matched"].([]string)[1]
			spouseType := model.Type(keyword)
			if spouseType.String() == "" {
	...
	engine.OnRegex(`^抽\s?(.*)$`, zero.OnlyGroup).SetBlock(true).Limit(ctxext.LimitByGroup).
		Handle(func(ctx *zero.Ctx) {
			keyword := ctx.State["regex_matched"].([]string)[1]
			spouseType := model.Type(keyword)
			if spouseType.String() == "" {
	...
	engine.OnRegex(`^(.*)列表$`, zero.OnlyGroup).SetBlock(true).Limit(ctxext.LimitByGroup).
		Handle(func(ctx *zero.Ctx) {
			keyword := ctx.State["regex_matched"].([]string)[1]
			spouseType := model.Type(keyword)
			if spouseType.String() == "" {
	...
	engine.OnRegex(`^查看\s?(.*)$`, zero.OnlyGroup).SetBlock(true).Limit(ctxext.LimitByGroup).
		Handle(func(ctx *zero.Ctx) {
			keyword := ctx.State["regex_matched"].([]string)[1]
			spouseType := model.Type(keyword)
			if spouseType.String() == "" {
